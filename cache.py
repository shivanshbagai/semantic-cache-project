...
# 'department' is declared as a tag field so tenant isolation is enforced
# inside the Redis ANN query, not post-hoc in Python.
llm_cache = SemanticCache(
    name="ollama_semantic_cache",
    redis_url="redis://localhost:6379",
    vectorizer=vectorizer,
    distance_threshold=0.30,
    filterable_fields=[{"name": "department", "type": "tag"}],
)
